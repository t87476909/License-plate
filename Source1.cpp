...
Mat frame;                              // ?前?
Mat fgMaskMOG2;                         // 前景模板，通?MOG2方法生成
...
void svm_train(Ptr<SVM> &mode2, Mat &trainData, Mat &trainLabels) //??SVM
{
	mode2->setType(SVM::C_SVC);     //SVM?型
	mode2->setKernel(SVM::LINEAR);  //核函?，?里使用?性核
...
void svm_pridect(Ptr<SVM> &mode2, Mat test) //利用??好的SVM??
{
	Mat result;
...
int main()
{
	...
	while (true)
	{
		...
		img++;
		resize(videoFrame, videoFrame, Size(960, 540));//960 540 //1280 720 //1.77777*9
		...
		if ((img % a) == 0 && !videoFrame.empty())
		{
			...
			for (int i = 0; i < 107; i++) //負樣本 sa.jpg+1 = 106 個
				lable[i] = 0;
			for (int j = 107; j < 145; j++) //正樣本  test+1-(sa.jpg +1) = 40個
				lable[j] = 1;
			Mat trainimage;
			transform(image, trainimage);
			Mat lableMat(145, 1, CV_32SC1, lable); //145 test+1 */
			...
			for (int i = 0; i < contours.size(); i++)
			{
				...
				if ((float)r.width / r.height >= 2.0 && (float)r.width / r.height <= 4.0) //2.0 3.6
				{
